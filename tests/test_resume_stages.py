...
from run_q_station_pipeline import QStationState, Runner
...
def _content_project():
    ...
    return load_content_project("q_station")
...
def _png(path: Path, *, seed: int = 1) -> Path:
    ...
    assert path.stat().st_size >= qstation.MIN_IMAGE_BYTES
    return path
...
def _mp4(path: Path, seconds: float = 1.6) -> Path:
    ...
    assert path.stat().st_size >= qstation.MIN_VIDEO_BYTES
    return path
...
def runner(project: Path) -> Runner:
    state = QStationState(project, "901_resume", "resume topic")
...
def image_receipt(project, target, prompt, references=(), model=qstation.CHATGPT_IMAGE_MODEL):
    ...
    qstation.save_json(project / 'pipeline/provider_receipts' / (name+'.json'),payload)
...
def prepare_beat_cache(runner, project, count):
    ...
    try:
        for number in range(1,count+1):
            beat={'beat_id':number}
            refs=qstation._beat_reference_stack(_content_project(),beat,
                project/'references/world_style_anchor.png',project/'references/world_keyframe.png',previous)
            prompt=qstation.stage_beat_prompt(runner,project,_content_project(),beat,{'medium':'woodcut'},refs)
...
def test_book_design_sheet_is_reused_when_the_canonical_asset_exists(
    ...
    monkeypatch.setattr(qstation, "book_design_sheet_path", lambda content_project: sheet)
    reviews = []
    monkeypatch.setattr(
        runner,
    ...
    result = qstation.stage_book_design_sheet(runner, project, _content_project())
    assert result == sheet
    ...
    assert qstation.stage_book_design_sheet(runner, project, _content_project()) == sheet
...
def test_book_spread_is_reused_after_a_restart(runner: Runner, project: Path) -> None:
    ...
    result = qstation.stage_book_spread(
...
def test_book_spread_recomposes_when_the_frame_is_missing(
    ...
    result = qstation.stage_book_spread(runner, project, world, {"book_template_id": "002"})
...
def test_flow_clips_are_reused_after_a_restart(
    ...
    result = qstation.stage_flow_clip(
...
def test_validated_body_images_are_reused(runner: Runner, project: Path) -> None:
    ...
    produced = qstation.stage_body_images(
...
def test_isolated_body_image_revision_preserves_every_other_image_byte_for_byte(
    ...
    prepare_beat_cache(runner, project, 3)
    before = {beat_id: qstation.sha256_file(target) for beat_id, target in targets.items()}
    calls: list[str] = []
    ...
    assert [path.name for path in produced] == ["beat_001.png", "beat_002.png", "beat_003.png"]
    assert qstation.sha256_file(targets[1]) == before[1]
    assert qstation.sha256_file(targets[2]) != before[2]
    assert qstation.sha256_file(targets[3]) == before[3]
...
def test_chatgpt_revision_guidance_reviews_old_image_once_and_reuses_receipt(
    ...
    first, first_receipt = qstation.chatgpt_revision_guidance(runner, project, 2, "original direction", request)
    second, second_receipt = qstation.chatgpt_revision_guidance(runner, project, 2, "original direction", request)
...
def test_stage_state_survives_a_fresh_state_object(project: Path) -> None:
    ...
    first = QStationState(project, "901_resume", "topic")
    first.mark("world_keyframe", qstation.STATE_DONE, sha256="abc")

    reloaded = QStationState(project, "901_resume", "topic")
