...
Write one Flow prompt under 500 characters for the question-spark opening. Return plain text only.
...
The uploaded `character_sheet` is identity-only and the only ingredient. Describe the resolved character's immutable appearance, behavior, negative constraints, episode-specific location/environment/activity, curiosity trigger, readable physical action and restrained camera. The host is already acting at frame zero.
