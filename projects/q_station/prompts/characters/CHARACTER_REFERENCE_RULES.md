...
- Flow Intro B uses Frames mode only: the profile-specific entry frame as `first_frame`, host-free WORLD_KEYFRAME as `last_frame`. It receives no character or style-sheet ingredient; any required ownership cue is already baked into the first frame.
