...
def launch_schema(projects: list[dict[str, str]], styles: list[str]) -> dict[str, Any]:
    ...
    groups = [
        ...
        {"id": "branding", "title": "Logo & video title", "description": "Persistent channel branding burned into the final video. Leave the title text empty to use the episode question/topic.", "fields": [
            ...
            _field("show_watermark", "Show text watermark", "toggle", default=False, help="A separate persistent text layer; it can be placed independently from the logo and title."),
            _field("watermark_text", "Watermark text", "text", default="", maxLength=220, placeholder="© Your Channel", requires={"field": "show_watermark", "value": True}),
            _field("watermark_position", "Watermark position", "select", default="bottom_right", options=select([("below_logo", "Below the logo (dynamic)"), *OVERLAY_POSITIONS]), requires={"field": "show_watermark", "value": True}),
            _field("watermark_font", "Watermark font", "select", default="Roboto", options=[
                {"value": name, "label": name, "recommended": name in RECOMMENDED_SUBTITLE_FONTS}
                for name in ALL_SUBTITLE_FONTS
            ], searchable=True, help="Uses the same installed and uploaded font library as subtitles and titles.", requires={"field": "show_watermark", "value": True}),
            _field("watermark_font_size", "Watermark font size", "number", default=28, min=TITLE_FONT_SIZE_MIN, max=TITLE_FONT_SIZE_MAX, step=1, width="half", requires={"field": "show_watermark", "value": True}),
            _field("watermark_opacity", "Watermark opacity", "number", default=.72, min=.05, max=1, step=.05, width="half", requires={"field": "show_watermark", "value": True}),
            _field("watermark_max_words_per_line", "Maximum words per line", "number", default=7, min=1, max=100, step=1, width="half", help="Set this high enough to keep the complete watermark on one line.", requires={"field": "show_watermark", "value": True}),
            _field("watermark_line_spacing", "Line spacing (px)", "number", default=6, min=-10, max=100, step=1, width="half", requires={"field": "show_watermark", "value": True}),
            _field("watermark_max_width_percent", "Watermark box width %", "number", default=34, min=12, max=90, step=1, width="half", requires={"field": "show_watermark", "value": True}),
            _field("watermark_bold", "Watermark bold", "toggle", default=True, requires={"field": "show_watermark", "value": True}),
            _field("watermark_italic", "Watermark italic", "toggle", default=False, requires={"field": "show_watermark", "value": True}),
            _field("watermark_text_align", "Watermark text alignment", "select", default="right", options=select([("left", "Left"), ("center", "Centre"), ("right", "Right")]), requires={"field": "show_watermark", "value": True}),
            _field("watermark_margin_x_percent", "Watermark horizontal margin %", "number", default=3, min=0, max=25, step=.5, width="half", requires={"field": "show_watermark", "value": True}),
            _field("watermark_margin_y_percent", "Watermark vertical margin %", "number", default=2.5, min=0, max=25, step=.5, width="half", requires={"field": "show_watermark", "value": True}),
            _field("watermark_logo_gap_percent", "Gap below logo %", "number", default=1, min=0, max=15, step=.5, width="half", requires=[{"field": "show_watermark", "value": True}, {"field": "watermark_position", "value": "below_logo"}]),
            _field("watermark_custom_x_percent", "Watermark box centre X %", "number", default=80, min=0, max=100, step=.5, width="half", requires=[{"field": "show_watermark", "value": True}, {"field": "watermark_position", "value": "custom"}]),
            _field("watermark_custom_y_percent", "Watermark box top Y %", "number", default=88, min=0, max=100, step=.5, width="half", requires=[{"field": "show_watermark", "value": True}, {"field": "watermark_position", "value": "custom"}]),
            _field("watermark_font_colour", "Watermark colour", "color", default="#FFFFFF", width="half", presets=list(SUBTITLE_COLOUR_PRESETS), requires={"field": "show_watermark", "value": True}),
            _field("watermark_outline_colour", "Watermark outline colour", "color", default="#000000", width="half", presets=list(SUBTITLE_COLOUR_PRESETS), requires={"field": "show_watermark", "value": True}),
            _field("watermark_outline", "Watermark outline width", "number", default=2, min=0, max=8, step=.5, requires={"field": "show_watermark", "value": True}),
        ]},
        ...
        {"id": "visual", "title": "World style & still images", "description": "World identity, style selection and one provider for every generated still image.", "projects": ["q_station"], "fields": [
            ...
            _field("image_provider", "Image generation provider", "select", default="chatgpt", options=select([
                ("chatgpt", "ChatGPT · project chat · Extra High"),
                ("gemini", "Gemini · via Ordak"),
            ]), help="Applies to every generated still: style anchor, world keyframe, entry art, book/portal art, body shots and release thumbnail. Provider fallback is never automatic."),
            _field("gemini_image_model", "Gemini image model", "select", default="nano_banana_2", options=select([
                ("nano_banana_2", "Nano Banana 2"),
                ("nano_banana_pro", "Nano Banana Pro (availability required)"),
            ]), requires={"field": "image_provider", "value": "gemini"}, hideWhenGated=True),
            _field(
        ...
        {"id": "providers", "title": "Provider contract", "description": "Text and video are fixed; one image provider is frozen per run.", "projects": ["q_station"], "collapsed": True, "fields": [
            _field("locked_text", "Text", "readonly", default="ChatGPT · via Ordak"),
            _field("locked_image", "Image", "readonly", default="Selected per run: ChatGPT or Gemini · no fallback"),
